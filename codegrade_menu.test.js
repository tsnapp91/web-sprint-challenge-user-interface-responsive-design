...
describe("menu.html", () => {
  ...
  it("renders five menu-section class sections with proper titles in correct order", () => {
    const menuSections = Array.from(
      container.querySelectorAll(".menu-section")
    );
    ...
    for (let i = 0; i < menuSections.length; i++) {
      if (menuSections[i].innerHTML.includes("Drinks")) {
        ...
        expect(
          getByText(menuSections[2], /Soup and Salad/i)
        ).toBeInTheDocument();
  ...
  it("renders three to five menu-items per menu-section", () => {
    ...
    if (
      ...
      fail(`Inappropriate number of menu sections.`);
    ...
    menuSections.forEach((section) => {
      ...
      if (
        ...
        fail(`Inappropriate number of menu items.`);
